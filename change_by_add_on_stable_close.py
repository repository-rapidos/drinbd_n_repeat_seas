...
def df_csv_saver_append(df, filename, save_index = False):
	assert filename.endswith(".csv"), 'filename.endswith(".csv")'
	df.to_csv(filename, mode = 'a', header = not check_file_exists(filename), index = save_index)
...
def add_rows(df, nbr_add):
	df = df.copy()
	part_2_add = df.tail(nbr_add)
	part_2_add.reset_index(inplace = True, drop = True)
	df = pd.concat([df, part_2_add], axis = 0)
	df.reset_index(inplace = True, drop = True)
	return df
...
def manage(account,
		...
		drop_last_row_to_stabilize,
		start_runing_timestamp,
		nbr_add_rows,
	...
	### DELETE THE LAST CLOSE TO MAKE STABLE:
	###__________________________________
	df = df.head(df.shape[0] - drop_last_row_to_stabilize)
	df.reset_index(inplace = True, drop = True)

	### ADD ROWS:
	###__________
	df = add_rows(df = df, nbr_add = nbr_add_rows)
	### Print df original:
	...
	get_signals = f"/content/drive/MyDrive/get_signals/"
	try:
		os.mkdir(get_signals)
	except:
		pass
	...
	os_minute_now = int(datetime.datetime.now().minute) - drop_last_row_to_stabilize
	if int(reference_date_open_close.tail(1)['date_from'].tolist()[-1].split(" ")[1].split(":")[1]) == os_minute_now:
		...
		if use_tests_53_models_nbr is None:
			...
			concerned_date_from = df_results.tail(1)['date_from'].tolist()[0]
			df_results_last_row = df_results[df_results['date_from'] == concerned_date_from]
			df_results_last_row = df_results_last_row.head(1)
			df_results_last_row_filename = get_signals + "change_by_add_on_stable_close_" + str(int(start_runing_timestamp)) + ".csv"
			df_csv_saver_append(df = df_results_last_row, 
							filename = df_results_last_row_filename)
