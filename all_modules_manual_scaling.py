...
## storage drinbd-n-repeat-seas:
# https://console.firebase.google.com/project/saving-data-2ee4b/storage/saving-data-2ee4b.appspot.com/files/~2Fdrinbd-n-repeat-seas
...
def manage(test_nbr,
	...
	###___________________________
	"""
	# def get_extremum_in_df(df, type_extremum):
	# 	df = df.copy()
	# 	columns = df.columns.tolist()
	...
	# 			extrema.append(df[col].min())
	# 		elif type_extremum == 'max':
	# 			extrema.append(df[col].max())
	...
	# ### TRAIN DATA:
	# ###____________
	# min_in_train = get_extremum_in_df(df = df_train, type_extremum = 'min')
	# max_in_train = get_extremum_in_df(df = df_train, type_extremum = 'max')

	# min_in_test = get_extremum_in_df(df = df_test, type_extremum = 'min')
	# max_in_test = get_extremum_in_df(df = df_test, type_extremum = 'max')

	# print("min in train :", min_in_train)
	# print("max in train :", max_in_train)
	# print("min in test  :", min_in_test)
	...
	df_train_scaled = manual_scaler(df = df_train)

	### TEST DATA:
	###___________
	# df_test_scaled, df_test_scaled_scaler = data_scaler(df = df_test)
	df_test_scaled = manual_scaler(df = df_test)
