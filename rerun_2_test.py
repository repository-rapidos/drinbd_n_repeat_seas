...
def manage(test_nbr,
	...
	for step in range(1, steps+1):
		...
		if step == steps:
			...
			reference_date_open_close.reset_index(inplace = True, drop = True)
			# print("shape of reference_date_open_close :", reference_date_open_close.shape)
		...
		if step == steps:
			...
			complete_df_test_scaled = complete_df_test_scaled.tail(reference_date_open_close.shape[0] + look_back -1)
			complete_df_test_scaled.reset_index(inplace = True, drop = True)
			# print("complete_df_test_scaled shape :", complete_df_test_scaled.shape)
		...
		if step == steps:
			...
			df_results_direct = df_results.tail(last_data)
			direct_df_filename = f"direct_df_results_test_nbr_{test_nbr}_trial_nbr_{trial_nbr}.csv"
			# print("df_results_direct shape :", df_results_direct.shape)
			df_results_direct.to_csv(simulating_realtime_path + direct_df_filename)
	...
	# # indirect_df_y_preds = pd.DataFrame({'y_pred':y_preds})
	...
	# files.download(simulating_realtime_path + total_df_results_filename)
	files.download(simulating_realtime_path + balance_sheet_filename)
	files.download(simulating_realtime_path + direct_df_filename)
