...
def manage(account,
	...
	if int(reference_date_open_close.tail(1)['date_from'].tolist()[-1].split(" ")[1].split(":")[1]) == os_minute_now:
		...
		if use_tests_53_models_nbr is None:
			...
			after_concerned_date_from = df_results.tail(1)['date_from'].tolist()[0]
			concerned_index = df_results[df_results['date_from'] == after_concerned_date_from].index[0] - 1
			df_results_last_row = df_results[df_results.index == concerned_index]

			df_results_last_row_filename = get_signals + "change_by_add_on_instable_close_" + str(int(start_runing_timestamp)) + ".csv"
